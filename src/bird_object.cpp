...
#include <iostream>
    ...
    bird_object::bird_object(int a, int b, int c, int d, std::string e, int f, int g, int h) : 
    game_object(a, b, c, d, e), vel_x(0), vel_y(0), acel_x(0), acel_y(0), VEL_MAX(f), VEL_MIN(g), JUMP_VEL(h), angle(0){};

    void bird_object::Update(){
    ...
    void bird_object::Draw(){
        ALLEGRO_BITMAP* Bitmap=al_load_bitmap(this->get_sprite().c_str());
        abs_pos* drw_pos = this->Get_position();
        if (vel_y<0){
            angle=((-vel_y*(8.0/(18.0*VEL_MAX))*ALLEGRO_PI)+angle)/2.0;
        }else if(vel_y>0){
            angle=((vel_y*(1.0/(2.0*VEL_MIN))*ALLEGRO_PI)+angle)/2.0;
        }
        al_draw_scaled_rotated_bitmap(Bitmap,drw_pos->x+(drw_pos->w/2),drw_pos->y+(drw_pos->h/2),drw_pos->x,drw_pos->y,0.15,0.15,angle,0);
    }
