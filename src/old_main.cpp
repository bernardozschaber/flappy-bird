...
const char * BIRD_SPRITE = "assets/bird-1.png";                     // caminho do sprite do pássaro
...
int main(int argc, char **argv) 
{
    ...
    *mouse_install = al_install_mouse();        // Instalação do mouse
    ...
    int mouse_click_pos_u_x;                                                // Armazena a posição X de quando solta o mouse
    int mouse_click_pos_y;                                                  // Armazena a posição Y do clique do mouse
    int mouse_click_pos_u_y;                                                // Armazena a posição Y de quando solta o mouse
    ...
    while(open)
    {
        ...
        switch(event.type) 
        {
            ...
            case ALLEGRO_EVENT_MOUSE_BUTTON_UP:
                if (event.mouse.button & 1) // Botão esquerdo
                {
                    mouse_click_pos_u_x = event.mouse.x;
                    mouse_click_pos_u_y = event.mouse.y;
                }
                break;
        }
